...
def update_anchor_led(led, anchor_watch, now: float):
    ...
    >90% Radius:
      gelb schnelles 50%-Blinken

    >100% Radius:
      Polizei-Strobe rot/blau

    >Radius+Puffer:
      Polizei-Strobe bleibt, Buzzer separat
    """
    ...
    if zone == "warning":
        # Gelb: 50% Duty Cycle, deutlich aber nicht hektisch.
        phase = now % 0.6
    ...
    # Polizei-artiger Strobe:
    # rot blink-blink-blink, kurze Pause,
    # blau blink-blink-blink, kurze Pause.
    phase = now % 1.2

    if phase < 0.48:
        # roter Burst: 3 schnelle Pulse
        sub = phase % 0.16
        if sub < 0.08:
            led.set_color("red")
        else:
...
def make_button_handler(
    ...
    def handle_button(name: str):
        ...
        if state.mode == "pages":
            ...
            # Aus: OK setzt Drop. BACK geht nur zurück.
            ...
            # Drop gesetzt: OK setzt Radius. BACK bricht ab.
            ...
            # Aktiv/Alarm:
            # UP = Radius, DOWN = Puffer, OK = Stop?, BACK = zurück, Wache läuft.
